...
async def test_add_item_success(client, customer_token, storefront_data, mock_redis):
    ...
    response = await client.post(
        "/api/v1/cart/add", json=payload, headers=customer_token
    )
...
@pytest.mark.asyncio
async def test_add_rx_product_no_stock_fails(
    client, customer_token, storefront_data, mock_redis
):
...
@pytest.mark.asyncio
async def test_increment_existing_item(
    client, customer_token, storefront_data, mock_redis
):
...
async def test_add_item_insufficient_stock(
    ...
    payload = {"product_id": product_id, "quantity": 101}
...
async def test_removes_product(client, customer_token, storefront_data, mock_redis):
    ...
    response = await client.delete(
        f"/api/v1/cart/remove/{product_id}", headers=customer_token
    )
