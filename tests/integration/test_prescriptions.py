...
@pytest.mark.asyncio
async def test_upload_prescription_success(
    client, customer_token, sample_order, mock_storage_service
):
...
@pytest.mark.asyncio
async def test_pharmacist_list_pending(
    client, pharmacist_token, db_session, test_customer, sample_order
):
    ...
    response = await client.get(
        "/api/v1/prescriptions/pending", headers=pharmacist_token
    )
...
@pytest.mark.asyncio
async def test_approve_prescription_flow(
    client, pharmacist_token, db_session, test_customer, sample_order
):
    from app.models.prescription import Prescription
...
@pytest.mark.asyncio
async def test_reject_prescription_flow(
    client, pharmacist_token, db_session, test_customer, sample_order
):
    ...
    payload = {"prescription_id": str(presc.id), "reason": "Image is too blurry"}

    response = await client.post(
        "/api/v1/prescriptions/reject", json=payload, headers=pharmacist_token
    )
