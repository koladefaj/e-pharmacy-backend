...
class AdminPharmacistService:
    ...
    async def register_pharmacist(
        self, user_in: dict, background_tasks: BackgroundTasks
    ) -> User:
        ...
        try:
            ...
            if not new_pharmacist.license_verified:
                ...
                msg = (
                    f"Hi {new_pharmacist.full_name}, your pharmacist account is ready!"
                )
    ...
    async def deactivate_pharmacist_by_email(self, email: str) -> None:
        ...
        if not user:
            ...
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Pharmacist not found"
            )
    ...
    async def approve_pharmacist_account(
        self,
        pharmacist_id: UUID,
        approve_data: PharmacistApproveSchema,
        admin_email: str,
    ):
        # Fetch
        ...
        updated_pharmacist = await self.user_crud.verify_pharmacist(
            db_obj=pharmacist, obj_in=approve_data
        )

        # Audit Log
        logger.info(
            f"AUDIT: Pharmacist {pharmacist.email} approved by Admin {admin_email}"
        )
